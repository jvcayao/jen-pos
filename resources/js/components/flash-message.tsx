...
export default function FlashMessage() {
    ...
    return (
        <div
            className={`fixed top-6 left-1/2 z-50 -translate-x-1/2 rounded px-4 py-2 text-white shadow-lg transition-all ${type === 'success' ? 'bg-green-600' : type === 'error' ? 'bg-red-600' : 'bg-blue-600'}`}
            role="alert"
        >
            {message}
        </div>
