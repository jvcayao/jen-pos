...
import { ShoppingBag, Trash2, ImageIcon } from 'lucide-react';
...
export default function CartSidebar({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  function fetchCart() {
    ...
    router.get('/cart', {}, {
      onSuccess: (page) => {
        if (page.props.cart && Array.isArray(page.props.cart.items)) {
  ...
  function updateQty(id: number, qty: number, type: string) {
    setLoading(true);
    router.post('/cart/update', { id, qty, type }, {
      onSuccess: fetchCart,
      onError: () => setLoading(false),
    });
  }
  ...
  return (
    <>
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="right" className="p-0 max-w-sm w-full">
          <SheetHeader className="flex flex-row items-center justify-between px-6 py-4 border-b">
            <SheetTitle className="flex items-center gap-2 text-lg font-semibold">
              <ShoppingBag className="h-5 w-5" /> Cart ({cart.count})
            </SheetTitle>
            
          </SheetHeader>
          <div className="flex flex-col gap-2 p-6 overflow-y-auto h-[calc(100vh-120px)]">
            {loading ? (
              ...
              <Card key={item.id} className="mb-2">
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle className="truncate text-base font-medium">{item.name}</CardTitle>
                ...
                <CardContent className="flex items-center gap-2 py-2">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Button size="sm" variant="outline" onClick={() => updateQty(item.id, 1, 'decrease')} disabled={item.qty <= 1}>-</Button>
                      <span className="px-2 text-sm">{item.qty}</span>
                      <Button size="sm" variant="outline" onClick={() => updateQty(item.id, 1, 'increase')}>+</Button>
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="border-t px-6 py-4 flex flex-col gap-2">
            ...
            <Button className="w-full mt-2" disabled={cart.items.length === 0}>Checkout</Button>
          </div>
        </SheetContent>
      </Sheet>
    </>
