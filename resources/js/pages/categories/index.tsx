...
function CategoryItem({ node }: { node: CategoryNode }) {
  ...
  const createChild = useForm({ name: '', slug: '' as string | null, parent_id: Number(node.id) });
  ...
  const onDelete = () => {
    ...
    // Inertia delete
    editForm.delete(categoriesDestroy(node.id).url, { preserveScroll: true });
  };

  return (
    <div className="border border-sidebar-border/60 dark:border-sidebar-border rounded-lg">
      <div className="flex items-center gap-2 p-3">
        <button
          className={`inline-flex h-6 w-6 items-center justify-center rounded hover:bg-muted ${!hasChildren ? 'opacity-0 pointer-events-none' : ''}`}
          onClick={() => setOpen((o) => !o)}
        ...
        <div className="flex min-w-0 flex-1 items-center justify-between gap-2">
          <div className="min-w-0">
            {editing ? (
              <form onSubmit={onUpdate} className="flex flex-col gap-1 md:flex-row md:items-center md:gap-2">
                <input
                ...
                />
                <button className="h-8 rounded bg-primary px-3 text-xs text-primary-foreground">Save</button>
                <button type="button" onClick={() => setEditing(false)} className="h-8 rounded border px-3 text-xs">Cancel</button>
              </form>
            ) : (
              <>
                <div className="truncate font-medium">{node.name}</div>
                <div className="truncate text-xs text-muted-foreground">/{node.slug}</div>
              </>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setAddingChild((s) => !s)} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-xs">
              <Plus className="h-3 w-3" /> Sub-category
            </button>
            <button onClick={() => setEditing((s) => !s)} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-xs">
              <Pencil className="h-3 w-3" /> Edit
            </button>
            <button onClick={onDelete} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-xs text-red-600">
      ...
      {addingChild && (
        <div className="border-t border-sidebar-border/60 p-3 dark:border-sidebar-border">
          <form onSubmit={onCreateChild} className="flex flex-col gap-2 md:flex-row md:items-center md:gap-2">
            <input
              className="h-8 w-full rounded border border-input bg-background px-2 text-sm md:w-56"
              placeholder="Sub-category name"
              value={createChild.data.name}
              onChange={(e) => createChild.setData('name', e.target.value)}
              required
            />
            <input
              className="h-8 w-full rounded border border-input bg-background px-2 text-sm md:w-56"
              ...
              onChange={(e) => createChild.setData('slug', e.target.value)}
            />
            <button className="h-8 rounded bg-primary px-3 text-xs text-primary-foreground">Add</button>
            <button type="button" onClick={() => setAddingChild(false)} className="h-8 rounded border px-3 text-xs">Cancel</button>
          </form>
          {createChild.errors.name && (
            <div className="mt-1 text-xs text-red-600">{createChild.errors.name}</div>
          )}
        </div>
      )}

      {hasChildren && open && (
        <div className="border-t border-sidebar-border/60 dark:border-sidebar-border p-3">
  ...
  return (
    <div className="rounded-md border border-sidebar-border/60 p-3 dark:border-sidebar-border">
      <div className="mb-2 flex items-center gap-2">
        <FolderTree className="h-4 w-4 text-muted-foreground" />
        {editing ? (
          <form onSubmit={onUpdate} className="flex flex-1 flex-col gap-1 md:flex-row md:items-center md:gap-2">
            <input
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`#/products?category=${encodeURIComponent(String(node.id))}`} className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
            <LinkIcon className="h-3 w-3" /> Products
          </Link>
        </div>
      </div>
...
export default function CategoriesIndex() {
  ...
  const submitRoot = (e: React.FormEvent) => {
    e.preventDefault();
    createRoot.post(categoriesStore().url, { preserveScroll: true, onSuccess: () => createRoot.reset() });
  };
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        ...
        <div className="rounded-lg border border-sidebar-border/60 p-4 dark:border-sidebar-border">
          <form onSubmit={submitRoot} className="flex flex-col gap-2 md:flex-row md:items-center md:gap-2">
            <input
              ...
              onChange={(e) => createRoot.setData('name', e.target.value)}
              required
            />
            <input
              ...
              onChange={(e) => createRoot.setData('slug', e.target.value)}
            />
            <button className="h-9 rounded bg-primary px-3 text-sm text-primary-foreground">
              <Plus className="mr-1 inline-block h-4 w-4" /> Add main category
            </button>
          </form>
          {createRoot.errors.name && (
            <div className="mt-1 text-xs text-red-600">{createRoot.errors.name}</div>
          )}
        </div>
